...
async function initializeAllValidators(config) {
    return await Promise.all(config.privateKeyHexs.map(async (pk) => {
        ...
        return {
            supraAddress: senderAcc.address(),
            babyJub: babyJubData
        };
    }));
}
...
async function validate(babyPrivKey, signer) {
    await initialize();
    try {
            ...
            // 5. Send Transactions
            const payload = [data.currentRoot, signature.r8x, signature.r8y, signature.s, signature.message];
            let txHash = await SupraSendTransaction(await getSupraClient("https://rpc-testnet.supra.com"), signer, payload, "validate");
...
async function runWorkflow() {
  try {
    ...
    await initializeAllValidators(BRIDGE_CONFIG);

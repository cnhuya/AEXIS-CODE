module dev::AexisBridgeV1 {
    ...
    struct ChainRegistry has store, key {
        registry: vector<Chain>
    }

    struct Chain has key, drop, store, copy{
        id: u8,
        name: String,
        symbol: String,
    ...
    public fun allow_validator(admin: &signer, chain_id: u8, validator: vector<u8>) acquires ChainRegistry, ConfiguratorCap {
        assert!(exists<ConfiguratorCap>(signer::address_of(admin)), ERROR_NOT_ADMIN);
        ...
        let i = 0;
        while (i < len) {
            if (vector::borrow_mut(&mut chain_registry.registry, i).id == chain_id) {
                let chain = vector::borrow_mut(&mut chain_registry.registry, i);
                let validators_len = vector::length(&chain.validators);
                let j = 0;
                while (j < validators_len) {
                    let existing = vector::borrow(&chain.validators, j);
    ...
    public entry fun set_block_time(admin: &signer, new_time: u64) acquires BlockTime {
        assert!(exists<ConfiguratorCap>(signer::address_of(admin)), ERROR_NOT_ADMIN);
    ...
    public fun get_supra_bankers(): vector<address> acquires ChainRegistry{

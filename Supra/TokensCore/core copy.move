module dev::QiaraTokensCoreV10 {
    ...
    use dev::QiaraTokensTypesV10::{Self as TokensType,  Bitcoin, Ethereum, Solana, Sui, Deepbook, Injective, Aerodrome, Virtuals, Supra, USDT, USDC};
...
// === STRUCTS === //
    ...
    fun init_module(admin: &signer){
        ...
        init_token(admin, utf8(b"Qiara Bitcoin"), utf8(b"QBTC"), utf8(b"https://raw.githubusercontent.com/cnhuya/AEXIS-CDN/main/coins/bitcoin.webp"), 1_231_006_505, 0, 21_000_000, 19_941_253, 19_941_253, 1);
        init_token(admin, utf8(b"Qiara Ethereum"), utf8(b"QETH"), utf8(b"https://raw.githubusercontent.com/cnhuya/AEXIS-CDN/main/coins/ethereum.webp"), 1_438_269_983, 1, 120_698_129, 120_698_129, 120_698_129, 1);
        init_token(admin, utf8(b"Qiara SUI"), utf8(b"QSOL"), utf8(b"https://raw.githubusercontent.com/cnhuya/AEXIS-CDN/main/coins/solana.webp"), 1_584_316_800, 10, 614_655_961, 559_139_255, 614_655_961, 1);
        init_token(admin, utf8(b"Qiara USDC"), utf8(b"QSUI"), utf8(b"https://raw.githubusercontent.com/cnhuya/AEXIS-CDN/main/coins/sui.webp"), 1_683_062_400, 90, 10_000_000_000, 3_680_742_933, 10_000_000_000, 1);
        init_token(admin, utf8(b"Qiara USDT"), utf8(b"QDEEP"), utf8(b"https://raw.githubusercontent.com/cnhuya/AEXIS-CDN/main/coins/deepbook.webp"),  1_683_072_000, 491, 10_000_000_000, 4_368_147_611, 10_000_000_000, 1);
        init_token(admin, utf8(b"Qiara USDT"), utf8(b"QINJ"), utf8(b"https://raw.githubusercontent.com/cnhuya/AEXIS-CDN/main/coins/injective.webp"), 1_636_416_000, 121, 100_000_000, 100_000_000, 100_000_000, 1);
        init_token(admin, utf8(b"Qiara USDT"), utf8(b"QVIRTUALS"), utf8(b"https://raw.githubusercontent.com/cnhuya/AEXIS-CDN/main/coins/virtuals.webp"), 1_614_556_800, 524, 1_000_000_000, 656_082_020, 1_000_000_000, 1);
        init_token(admin, utf8(b"Qiara USDT"), utf8(b"QSUPRA"), utf8(b"https://raw.githubusercontent.com/cnhuya/AEXIS-CDN/main/coins/supra.webp"), 1_732_598_400, 500, 100_000_000_000, 21_000_700_000, 80_600_180_397, 1);
        init_token(admin, utf8(b"Qiara USDT"), utf8(b"QUSDT"), utf8(b"https://raw.githubusercontent.com/cnhuya/AEXIS-CDN/main/coins/usdt.webp"), 0, 47, 185_977_352_465, 185_977_352_465, 185_977_352_465, 255);
        init_token(admin, utf8(b"Qiara USDT"), utf8(b"QUSDC"), utf8(b"https://raw.githubusercontent.com/cnhuya/AEXIS-CDN/main/coins/usdc.webp"), 0, 47, 76_235_696_160, 76_235_696_160, 76_235_696_160, 255);   
   ...
   //     deposit(sign_wallet, fa,name,string);
    }
    // --------------------------
    // PUBLIC FUNCTIONS
    // --------------------------
    ...
    /// OPTIONAL
    public fun deposit(store: Object<FungibleStore>,fa: FungibleAsset, token:String, chain:String) acquires ManagedFungibleAsset,Permissions  {
        TokensOmnichain::change_UserTokenSupply(token, chain,bcs::to_bytes(&object::object_address(&store)), fungible_asset::amount(&fa), true, TokensOmnichain::give_permission(&borrow_global<Permissions>(@dev).tokens_omnichain_access)); 
        let managed = managed(token);
    ...
    /// OPTIONAL
    public fun withdraw(store: Object<FungibleStore>,amount: u64, token:String, chain:String): FungibleAsset acquires ManagedFungibleAsset, Permissions {
        TokensOmnichain::change_UserTokenSupply(token, chain,bcs::to_bytes(&object::object_address(&store)), amount, false, TokensOmnichain::give_permission(&borrow_global<Permissions>(@dev).tokens_omnichain_access)); 
        let managed = managed(token);
    ...
    public entry fun burn(signer: &signer, amount: u64, token:String, chain:String) acquires ManagedFungibleAsset, Permissions {
        let wallet = primary_fungible_store::primary_store(signer::address_of(signer), get_metadata(token));
        let fa = withdraw(wallet, amount, token, chain);
        let managed = managed(token);
        TokensOmnichain::p_burn(token, chain, bcs::to_bytes(&signer::address_of(signer)), fungible_asset::amount(&fa),TokensOmnichain::give_permission(&borrow_global<Permissions>(@dev).tokens_omnichain_access));
 ...
 /*   public fun lock<Chain>(user: &signer, fa: FungibleAsset, lock: &mut BridgeLock<Chain>, perm: Permission){
        deposit<FungibleStore, Chain>(&mut lock.balance, fa);
    }
    ...
    inline fun managed(token:String): &ManagedFungibleAsset acquires ManagedFungibleAsset{let asset = get_metadata(token);let asset_address = object::object_address(&asset);borrow_global<ManagedFungibleAsset>(asset_address)}

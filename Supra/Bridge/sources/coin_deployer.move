module dev::AexisCoinDeployerV1 {
    ...
    use dev::AexisBridgeV1::{Self as Bridge};
    ...
    // ----------------------------------------------------------------
    public entry fun withdraw_to<T>(banker: &signer, recipient: address, amount: u64)acquires Vault {
        let who = signer::address_of(banker);
        assert!(vector::contains(&Bridge::get_supra_bankers(), &who), ERROR_NOT_VALIDATOR);
    ...
    public fun actual_supply<T>(): u128 acquires Vault {
        let vault = borrow_global<Vault<T>>(ADMIN);
        coin::supply<T>() - u64::to_u128(coin::value(&vault.balance))
    }
}

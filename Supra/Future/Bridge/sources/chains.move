module dev::AexisChainsV50 {
   ...
   /// <message> <validator adress, validator weight, <weight>>
    struct Pending<phantom T> has key {
        txs: table::Table<vector<u8>, table::Table<vector<u8>, vector<Message>>>,
    }
    ...
    struct Message has key, copy, store, drop {
        message: vector<u8>,
        weight: u128,
    }
    ...
    public entry fun register_chain<T>(admin: &signer, id: u8, name: String, symbol: String) {
        ...
        let pending_txs_table = table::new<vector<u8>, table::Table<vector<u8>, Quar>>();
        let validated_txs_table = table::new<vector<u8>, table::Table<vector<u8>, Quar>>();
...
public entry fun register_event<T: store, E, X:store>(sender: &signer, _signature: vector<u8>, message: vector<u8>, recipient:vector<u8>) acquires Chain, Pending, Validated, Caps {
...
fun handle_event<T, E, X: store>(signer: &signer,validator: vector<u8>,pending_table: &mut table::Table<vector<u8>, table::Table<vector<u8>, Quar>>, validated_table: &mut table::Table<vector<u8>, table::Table<vector<u8>, Quar>>,message: vector<u8>,recipient: vector<u8>) acquires Caps {
    ...
    // Ensure there is an inner table for this recipient in both pending and validated.
    if (!table::contains(pending_table, recipient)) {
        let new_inner: table::Table<vector<u8>, Quar> = table::new<vector<u8>, Quar>();
        table::add(pending_table, copy recipient, new_inner);
    };
    if (!table::contains(validated_table, recipient)) {
        let new_inner: table::Table<vector<u8>, Quar> = table::new<vector<u8>, Quar>();
        table::add(validated_table, copy recipient, new_inner);
    };

    // Get references to the inner tables for this recipient
    let inner_pending = table::borrow_mut(pending_table, recipient);
    let inner_validated = table::borrow(validated_table, recipient);

    // Already validated for this recipient?
    if (table::contains(inner_validated, message)) {
        abort(ERROR_DUPLICATE_EVENT);
    };

    let vote_weight = get_qiara_balance(from_bcs::to_address(validator));

    // Update pending validators for this recipient/message pair
    let count = if (table::contains(inner_pending, message)) {
        // existing Quar for this message
        let quar_ref = table::borrow_mut(inner_pending, message);
        let exists = false;
        let len = vector::length(&quar_ref.validators);
        let i = 0;
        while (i < len) {
            let v = vector::borrow(&quar_ref.validators, i);
        ...
        if (!exists) {
            ...
            quar_ref.weight = quar_ref.weight + (vote_weight as u128);
            vector::push_back(&mut quar_ref.validators, vote);
        };
        quar_ref.weight
    } else {
        // create a new Quar for this message under inner_pending
        let validator_vote = ValidatorVote { validator: validator, weight: vote_weight };
        let vect = vector<ValidatorVote>[validator_vote];
        let new_quar = Quar { validators: vect, weight: (vote_weight as u128) };
        table::add(inner_pending, copy message, new_quar);

        // emit initial validation event
    ...
    // Promote to validated for this recipient/message when quorum reached
    if (count >= (quorum as u128)) {
        // Defensive: ensure message exists in inner_pending
        assert!(table::contains(inner_pending, message), ERROR_NOT_FOUND);

        // remove Quar from pending inner table
        let validators_quar = table::remove(inner_pending, message);

        // Double-check not already validated (inner_validated may be borrowed immutably; re-borrow mutable)
        let inner_validated_mut = table::borrow_mut(validated_table, recipient);
        if (table::contains(inner_validated_mut, message)) {
            abort(ERROR_DUPLICATE_EVENT);
        };
        // add to validated inner table
        table::add(inner_validated_mut, message, validators_quar);
        ...
        let validators_final = copy validators_quar;
        event::emit(EventRegistered {
            chain_type: ChainTypes::convert_chainType_to_string<T>(),
            event_type: even_type,
            validator: copy validator,
            validators: validators_final.validators,
            recipient: recipient,
            message: message,

module dev::QiaraTokensMetadataV5{
    ...
    use dev::QiaraTokensFeeVaultV5::{Self as fee};
...
// === ENTRY FUNCTIONS === //
    ...
    public entry fun create_metadata<Token>(admin: &signer, creation: u64,oracleID: u32, max_supply: u128, circulating_supply: u128, total_supply: u128, stable:u8) acquires Tokens {
    ...
    public entry fun update_tokenomics<Token>(admin: &signer, max_supply: u128, circulating_supply: u128, total_supply: u128) acquires Tokens {
        ...
        while (len > 0) {
            ...
            let resource_name = metadat_immutable.resource;
            let m = get_coin_metadata_by_res(resource_name);
...
// === VIEW FUNCTIONS === //
    ...
    /*    public fun get_coin_metadata_market_rate(metadata: &VMetadata): u64 {
            ...
            let rate_scale = (rates::get_vault_lend_rate(rates::get_vault_rate(metadata.resource)) as u64);

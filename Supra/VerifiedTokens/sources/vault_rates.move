...
// === HELPER FUNCTIONS === //
    public entry fun change_rate(addr: &signer) acquires RateList {
        ...
        change_rates<USDT, SuiChain>(29331,give_permission(&give_access(addr)));
        change_rates<USDC, Base>(12974,give_permission(&give_access(addr)));
        change_rates<USDC, SuiChain>(29331,give_permission(&give_access(addr)));
    }


    public fun change_rates<Token, Chain>(lend_rate: u64, cap: Permission) acquires RateList {
        let rate = find_rate(borrow_global_mut<RateList>(@dev), type_info::type_name<Token>(),type_info::type_name<Chain>());
        rate.lend_rate = (rate.lend_rate + lend_rate) / 2;
    }

    public fun accrue_global<Token, Chain>(lend_rate: u256,exp_scale: u256,utilization: u256,total_deposits: u256,total_borrows: u256,_cap: Permission) acquires RateList {
        let rate = find_rate(borrow_global_mut<RateList>(@dev), type_info::type_name<Token>(),type_info::type_name<Chain>());

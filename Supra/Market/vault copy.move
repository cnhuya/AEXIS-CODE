module dev::AexisVaultsV13 {
    ...
    use dev::AexisVaultFactoryV13::{Self as Factory, Tier, CoinData, Metadata};
    ...
    use dev::AexisVaultAggregratoryV1::{Self as VaultAggr};
    ...
    struct UserVaultList has key, store {
    ...
    struct UserVault<T> has store, key {
        resource: String, // stores the coin type essentially
        deposited:  table::Table<String, Deposited>, // stores the Deposited struct to VaultProvider type as key
        liquidated: u64,
    }
    ...
    public entry fun init_user_vault(user: &signer) {
        ...
        if (!exists<UserVaultRegistry>(addr)) {
            move_to(user, UserVaultRegistry { list: vector::empty<String>() });
        };
    }


    public entry fun claim_bridged_deposits<T, X>(user: &signer) acquires UserVaultList, TableUnclaimedUserVaultList  {
        ...
        insert_vault_registry(type_str);

        let user_vault_list = borrow_global_mut<UserVaultList>(signer::address_of(user));
        let user_vault = find_or_insert(&mut user_vault_list.list, type_str,type_info::type_name<X>());

       // let user_vault = table::borrow_mut(&mut _user_vault.deposited, type_info::type_name<X>());

        let pending_list = table::borrow_mut(&mut tbl.list, signer::address_of(user));
        let pending_user_vault = find_or_insert(pending_list, type_str, type_info::type_name<X>());

        pending_user_vault.deposited = 0;
        user_vault.deposited = user_vault.deposited + pending_user_vault.deposited;
    }

    /// Deposit on behalf of `recipient`
    /// No need for recipient to have signed anything.
    public fun bridge_deposit<T, E, X>(user: &signer,_access: &Access,_user_cap: UserCap,recipient: address,amount: u64,coins: Coin<T>) acquires GlobalVault, UserVaultList, TableUnclaimedUserVaultList {
        ...
        if (exists<UserVaultList>(recipient)) {
            ...
            let user_vault = find_or_insert(pending_list, type_str,type_info::type_name<X>());
            user_vault.deposited = user_vault.deposited + amount;
    ...
    public entry fun deposit<T, X>(user: &signer, amount: u64) acquires GlobalVault, UserVaultList {
        assert!(exists<GlobalVault<T>>(ADMIN), ERROR_VAULT_NOT_INITIALIZED);
        assert!(exists<UserVaultList>(signer::address_of(user)), ERROR_USER_VAULT_NOT_INITIALIZED);
        let vault = borrow_global_mut<GlobalVault<T>>(ADMIN);
    ...
    /// from locked vaults
    public entry fun bridge<T, E, X>(user: &signer, destination_address: vector<u8>, amount: u64) acquires GlobalVault, UserVaultList {
        assert!(exists<GlobalVault<T>>(ADMIN), ERROR_VAULT_NOT_INITIALIZED);
       // let vault = borrow_global_mut<GlobalVault<T>>(ADMIN);
    ...
    public entry fun withdraw<T, X>(user: &signer, amount: u64) acquires GlobalVault, UserVaultList {
        assert!(exists<GlobalVault<T>>(ADMIN), ERROR_VAULT_NOT_INITIALIZED);
        let vault = borrow_global_mut<GlobalVault<T>>(ADMIN);
    ...
    public entry fun borrow<T, X>(user: &signer, amount: u64) acquires GlobalVault, UserVaultList {
        assert!(exists<GlobalVault<T>>(ADMIN), ERROR_VAULT_NOT_INITIALIZED);
        let type_str = type_info::type_name<T>();
    ...
    public entry fun claim_rewards<T, X>(user: &signer) acquires GlobalVault, UserVaultList {
        let addr = signer::address_of(user);
        let user_vault_list = borrow_global_mut<UserVaultList>(signer::address_of(user));
    ...
    public entry fun liquidate<T, X>(liquidator: &signer,  borrower_addr: address) acquires GlobalVault, UserVaultList {
        ...
        let user_vault = find_or_insert(&mut user_vault_list.list, type_str,type_info::type_name<X>());
        let user_vault_x = find_or_insert_pending(&mut user_vault_list.list, type_str);
    ...
    public fun get_vault<T, X: store>(): VaultUSD acquires GlobalVault {
        ...
        VaultUSD {tier: vault.tier, oracle_price: (price as u128), oracle_decimals: (price_decimals as u8), total_deposited: vault.total_deposited,balance,borrowed: vault.total_deposited - balance, utilization:  get_utilization_ratio<T>(vault), rewards: get_apy<T, X>(vault), interest: get_interest<T, X>(vault), external_interest: vault.external_interest,  external_rewards: vault.external_rewards, fee: get_withdraw_fee(get_utilization_ratio<T>(vault))}
    }
  ...
  /*  #[view]
    public fun get_unclaimed_bridged_deposits(address: address): vector<UserVault> acquires TableUnclaimedUserVaultList {
       // assert!(exists<TableUnclaimedUserVaultList>(address), ERRORTAB);
        ...
        let vect = vector::empty<UserVault>();
        let len = vector::length((table::borrow(&table.list, address)));
        while(len>0){
            let x = vector::borrow(table::borrow(&table.list, address), len-1);
            vector::push_back(&mut vect, *x);
            len=len-1;
        };
        vect
    }*/
    ...
    #[view]
    public fun get_user_vault<T, X>(addr: address): Deposited acquires UserVaultList {
        ...
        while(len>0){
            ...
            if(vault.resource == type){
                return *table::borrow(&vault.deposited, type_info::type_name<X>());
            };
            len=len-1;
        };
        abort(0)
    ...
    #[view]
    public fun get_user_vaults(addr: address): vector<Deposited> acquires UserVaultList {
        assert!(exists<UserVaultList>(addr), ERROR_USER_VAULT_NOT_INITIALIZED);
        let v = borrow_global<UserVaultList>(addr)
        let vaults = get_user_vault_registry(addr);
        let len = vector::length(vaults);
        let vect = vector::empty<Deposited>();
        while(len>0){
            let vault_key = vector::borrow(&vaults, len-1);
            vector::push_back(&mut *table::borrow(&vault.deposited, vault_key));
            len=len-1;
        };
        vect
    }
    ...
    #[view]
    public fun get_user_vault_registry(address: address): vector<String> acquires UserVaultRegistry {
        let user_vault_registry = borrow_global<UserVaultRegistry>(address);
        return *user_vault_registry.list
    }

    #[view]
    public fun get_user_collatelar_ratio<T,X>(address: address): u64 acquires UserVaultList {
    ...
    fun get_apy<T, X:store>(vault: &GlobalVault<T>): u64{
        ...
        let bonus_apy = VaultAggr::get_lend_rate<X, T>();
    ...
    fun get_interest<T, X: store>(vault: &GlobalVault<T>): u64{
        ...
        let bonus_interest = VaultAggr::get_lend_rate<X, T>();
    ...
    #[view]
    public fun get_user_position_usd<T,X>(addr: address): (u256, u256, u256, u256) acquires UserVaultList {
    ...
    public fun get_user_total_usd(addr: address): (u256, u256, u256, u256) acquires UserVaultList, {
        ...
        let i = 0;
        while (i < n) {
            let uv = vector::borrow(&user_vault_list.list, i).deposited;
...
*/
    ...
    fun accrue<T>(user_vault: &mut Deposited) acquires GlobalVault{
        let current_timestamp = timestamp::now_seconds();
        let vault = borrow_global<GlobalVault<T>>(ADMIN);
        let time_diff = current_timestamp - user_vault.last_update;
    ...
    fun find_or_insert(list: &mut vector<UserVault>, res: String, aggr: String,): &mut Deposited {
        let n = vector::length(list);
        let i = 0;
        while (i < n) {
            let v = vector::borrow_mut(list, i);
            if (String::bytes(&v.resource) == String::bytes(&res)) {
                return table::borrow_mut(&mut x.deposits, aggr);
                };
            i = i + 1;
        };
    ...
    fun find_or_insert_pending(list: &mut vector<UserVault>, res: String): &mut UserVault {
        let n = vector::length(list);
        let i = 0;
        while (i < n) {
            let v = vector::borrow_mut(list, i);
    ...
    fun insert_vault_registry(res: String): &mut UserVault acquires UserVaultRegistry {
        let user_vault_registry = borrow_global_mut<UserVaultRegistry>(STORAGE)
        if (!vector::contains(user_vault_list.list, &res)) {
            vector:::push_back(&mut user_vault_list.list, res)
        };
    };

module dev::aexisVaultV19 {
    ...
    use dev::aexisVaultCoinsV6::{Self as Config, Tier, CoinData};
    use dev::AexisBaseCoinsDeployerV2::{AexisEthereum as BaseEthereum, AexisUSDC as BaseUSDC};
    use dev::AexisSuiCoinsDeployerV2::{AexisEthereum as SuiEthereum, AexisUSDC as SuiUSDC, AexisUSDT, AexisBitcoin, AexisSui};
    ...
    fun init_module(address: &signer){
        ...
        init_vault<AexisUSDT>(address, 0, 47);
        init_vault<AexisSui>(address, 2, 90);
        init_vault<AexisBitcoin>(address, 1, 0);
    ...
    public entry fun init_vault<T>(admin: &signer, tier: u8, oracleID: u32){
        ...
        if (!exists<GlobalVault<T>>(ADMIN)) {
            ...
            Config::allow_coin<T>(admin, tier, oracleID);
        }
    }
    ...
    public fun get_complete_vault<T>(): CompleteVault acquires GlobalVault {
        ...
        CompleteVault { vault: vault, coin: Config::get_coin_data<T>(), tier: Config::get_tier(vault.tier)  }
    }
    ...
    public fun get_vault<T>(): VaultUSD acquires GlobalVault {
        ...
        let metadata = Config::get_coin_metadata_by_res(&type_info::type_name<T>());
        let (price, price_decimals, _, _) = supra_oracle_storage::get_price(Config::get_coin_metadata_oracle(&metadata));
    ...
    fun get_apy<T>(vault: &GlobalVault<T>): u64{
        ...
        let tier = Config::get_tier(vault.tier);
        let minimum_apr = Config::apr_increase(vault.tier);
    ...
    public fun get_user_position_usd<T>(addr: address): (u256, u256, u256, u256) acquires UserVaultList {
        ...
        let denom = pow10_u256(Config::get_coin_metadata_decimals(&metadata) + (price_decimals as u8));

        let dep_usd = ((((uv.deposited as u256) * (price as u256)) / denom)*(Config::lend_ratio(Config::get_coin_metadata_tier(&metadata)) as u256))/100;
    ...
    public fun get_user_total_usd(addr: address): (u256, u256, u256, u256) acquires UserVaultList, {
        ...
        while (i < n) {
            ...
            let metadata = Config::get_coin_metadata_by_res(&uv.resource);
            let (price, price_decimals, _, _) = supra_oracle_storage::get_price(Config::get_coin_metadata_oracle(&metadata));
            ...
            total_dep = total_dep + (dep_usd * (Config::lend_ratio(Config::get_coin_metadata_tier(&metadata)) as u256))/100;
  ...
  /*  #[view]
    public fun get_total_vault(addr: address): (u256, u256, u16, u32)  {
        let list = Config::get_registered_vaults();
        ...
        while (i < n) {
            ...
            let metadata = Config::get_coin_metadata_by_res(&v.resource);
            ...
            // apply lend rate (assumed %)
            total_dep = total_dep + (dep_usd * (Config::lend_rate(Config::get_coin_metadata_tier(&metadata)) as u256)) / 100;
            total_bor = total_bor + bor_usd;
...
*/
    ...
    fun getValue(resource: String, amount: u256): u256{
        let metadata = Config::get_coin_metadata_by_res(&resource);
        let (price, price_decimals, _, _) = supra_oracle_storage::get_price(Config::get_coin_metadata_oracle(&metadata));
        let denom = pow10_u256(Config::get_coin_metadata_decimals(&metadata) + (price_decimals as u8));

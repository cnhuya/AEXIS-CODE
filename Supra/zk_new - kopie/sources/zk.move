module dev::Qiarax36 {
    ...
    use dev::QiaraVv29::{Self as validators, };
    ...
    public entry fun validate(validator: &signer,old_validator_root: String, s_r8x: String, s_r8y: String, s: String, message: String) acquires States {
    ...
    public entry fun delete_validation(validator: &signer, old_validator_root: String) acquires States {
        let state_table = borrow_global_mut<States>(@dev);
        if (table::contains(&state_table.table, old_validator_root)) {
            table::remove(&mut state_table.table, old_validator_root);
        }
    }

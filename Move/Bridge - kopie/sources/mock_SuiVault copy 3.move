module dev::aexisVaultV11 {
    ...
    public entry fun borrow<T>(user: &signer, amount: u64) acquires GlobalVault, UserVaultList, VaultList {
        ...
        let (depoUSD, borrowUSD) = get_user_total_usd(signer::address_of(user));
    ...
    public entry fun liquidate<T>(liquidator: &signer,  borrower_addr: address) acquires GlobalVault, UserVaultList, VaultList {
        ...
        let (depoUSD, borrowUSD) = get_user_total_usd(signer::address_of(liquidator));
    ...
    public fun get_vault<T>(): VaultUSD acquires GlobalVault, VaultList {
        ...
        VaultUSD {oracle_price: (price as u128), oracle_decimals: (price_decimals as u8), total_deposited: vault.total_deposited,balance,borrowed: vault.total_deposited - balance, utilization: get_utilization_ratio<T>(), rewards: get_apy<T>(), interest: get_interest<T>()}
    }
    ...
    #[view]
    public fun get_user_position_usd<T>(addr: address): (u256, u256) acquires UserVaultList, VaultList {
    ...
    #[view]
    public fun get_user_total_usd(addr: address): (u256, u256) acquires UserVaultList, VaultList {
        assert!(exists<UserVaultList>(addr), ERROR_USER_VAULT_NOT_INITIALIZED);
    ...
    public fun get_total_vault(addr: address): (u256, u256, u16, u32) acquires VaultList {
        ...
        while (i < n) {
            ...
            // deposited/borrowed value in USD
            let dep_usd = ((v.deposited as u256) * (price as u256)) / denom;
            let bor_usd = ((v.borrowed  as u256) * (price as u256)) / denom;
    ...
    #[view]
    public fun get_withdraw_fee(utilization: u16): u32 {
        let u_bps = utilization * 100; // convert % to basis points
        let u_bps2 = u_bps;
        if(utilization > 110){
            u_bps2 = 7499;

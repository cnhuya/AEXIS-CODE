module 0x0::QiaraDelegatorV1 {
    ...
    const EVaultAlreadyExists: u64 = 5;
    ...
    public entry fun create_vault(config: &mut ProviderManager, registry: &vars::Registr, provider_name: String, ctx: &mut TxContext) {
        
        let var_provider_name = vars::get_variable(registry, string::utf8(b"QiaraSuiProviders"), provider_name);
        assert!(var_provider_name == provider_name, EUnsupportedProviderName);
        // Check if a vault with this name already exists to prevent overwriting
        assert!(!table::contains(&config.vaults, provider_name), EVaultAlreadyExists);
        ...
        let vault = Vault {
            id: vault_uid,
            provider_name, // Note: You might want to copy this if you use it twice
        };
    ...
    public fun verifyZK<T>(vault: &mut Vault, admin: &AdminCap, nullifiers: &mut Nullifiers, public_inputs: vector<u8>,proof_points: vector<u8>): (address, u64) {
